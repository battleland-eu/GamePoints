...
public class StoreView extends AbstractMenu<GamePoints> {
    ...
    private void replaceProduct(@NotNull ItemStack item, @NotNull IPointProduct product, @NotNull PointUser user) {
        ...
        String userProdCooldown = TimeUT.formatTimeLeft(user.getProductCooldown(product));
        ...
        formatProduct = StringUT.replace(formatProduct, IPointProduct.PLACEHOLDER_DESCRIPTION, false, product.getDescription());
        formatProduct = StringUT.replace(formatProduct, "%purchase%", false, formatState);
